...
def generate_data_save(h5data, _, time_to_reward_data, steps_to_reward_data, weights_data):
    ...
    h5data['steps_to_reward'].create_dataset(
        "steps_to_reward_{}".format(_), data=steps_to_reward_data, compression="gzip")
...
def generate_data_sets(filename, centers):
    ...
    h5data['neurons'].create_dataset("centers", data=centers, compression="gzip")
    return h5data
